...
// Источники кроме входа — комментарии (5.2), оценки (5.3) и принятые предложения (5.4) — используют ту же
// таблицу xp_events для дневных лимитов.
package xp
...
const (
	...
	SourceSuggestion Source = "suggestion"  // принятое предложение (шаг 5.4)
	SourceSecretCode Source = "secret_code" // погашенный скрытый код (шаг 5.5)
)
...
const (
	...
	SuggestionXP = 100 // за принятое предложение (спецификация §5)
	// Спецификация дневной лимит для предложений не называет (их принимают редко), но правило «лимит XP в сутки
	// на источник» (§5) относится и к ним: **предложено** — 3 принятых в день (проверяется тестом).
	SuggestionDailyCap = 3

	// SecretCodeDailyCap — дневной лимит Award() формально нужен, но каждый код погашается не больше
	// раза за всё время (гарантирует PRIMARY KEY таблицы редемпций), поэтому предел здесь — заглушка
	// «практически не ограничено», а не игровое правило.
	SecretCodeDailyCap = 1 << 30
)
