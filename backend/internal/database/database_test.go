...
func TestMigrateUpDownUp(t *testing.T) {
	...
	if v := version(); v != 5 {
		t.Fatalf("версия схемы %d, ожидалась 5", v)
	}
	...
	for _, tbl := range slices.Concat(accountTables, documentTables, roleTables, versionTables) {
		if !tableExists(tbl) {
	...
	for _, want := range []string{"0001_extensions.sql", "0002_accounts.sql", "0003_documents.sql", "0004_roles.sql", "0005_document_versions.sql", "применена"} {
		if !strings.Contains(out.String(), want) {
			t.Errorf("status не содержит %q: %q", want, out.String())
		}
	}

	// Откат — по одной миграции: версии и замки, роли, документы, аккаунты, расширения.
	if err := database.MigrateDown(ctx, db, log); err != nil {
		t.Fatal(err)
	}
	if v := version(); v != 4 {
		t.Fatalf("после первого down версия %d, ожидалась 4", v)
	}
	...
	if v := version(); v != 3 {
		t.Fatalf("после второго down версия %d, ожидалась 3", v)
	}
	...
	if v := version(); v != 2 {
		t.Fatalf("после третьего down версия %d, ожидалась 2", v)
	}
	...
	if v := version(); v != 1 {
		t.Fatalf("после четвёртого down версия %d, ожидалась 1", v)
	}
	...
	if v := version(); v != 5 || !extExists("citext") || !tableExists("users") || !tableExists("documents") || !tableExists("user_roles") || !tableExists("document_versions") {
