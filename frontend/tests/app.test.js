...
let server
let mode = 'ok' // ok | outage | maintenance
let hits = 0
...
beforeAll(async () => {
  ...
  vi.stubEnv('VITE_API_BASE', `http://127.0.0.1:${server.address().port}/api`)
})
...
async function mountApp(path = '/') {
  ...
  const { useConnectionStore } = await import('../src/stores/connection.js')

  // Реальные маршруты приложения + маршрут, которому нужен API (появятся с этапа 1).
...
async function retry(connection) {
...
describe('главная', () => {
  it('показывает «дверь»: название и гриф; примечание автора — только мелко в подвале, не на самой странице', async () => {
    await mountApp('/')
    expect(wrapper.find('h1').text()).toBe('Купол')
...
describe('сбой связи', () => {
  ...
  it('страница с API заменяется на «Сбой архива» с номером обращения', async () => {
    ...
    expect(wrapper.find('h1').text()).toBe('Сбой архива')
...
describe('обслуживание', () => {
  it('закрывает весь сайт, даже страницы без API', async () => {
    mode = 'maintenance'
    await mountApp('/')
    expect(wrapper.find('h1').text()).toBe('Архив закрыт на инвентаризацию')
...
describe('404', () => {
  it('неизвестный адрес -> «Дело не найдено», заголовок вкладки, ссылка на главную', async () => {
    ...
    expect(wrapper.find('h1').text()).toBe('Дело не найдено')
    expect(text()).toContain('Изъято')
    expect(document.title).toBe('Дело не найдено — КУПОЛ')
    expect(wrapper.find('a.btn').attributes('href')).toBe('/')
  })
