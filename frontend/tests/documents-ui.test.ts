...
describe('formatComposed', () => {
  ...
  it('пусто или мусор — пустая строка', () => {
    ...
    expect(formatComposed({ year: 0 })).toBe('')
  })
})

// RichRuns — компонент из нескольких корневых узлов; чтобы видеть текст как есть (с пробелами),
// его монтируют внутри абзаца.
const inParagraph = (runs: OutRun[]) => mount({ render: () => h('p', [h(RichRuns, { runs })]) })

describe('RichRuns', () => {
  const html = (runs: OutRun[]) => inParagraph(runs).html()
...
describe('BlockRenderer', () => {
  ...
  const render = (type: string, data: unknown = {}) => mount(BlockRenderer, { props: { block: { id: 'x', type, data } as DocBlock }, global: { stubs } })
  const run = (text: string): OutRun[] => [{ text }]
  ...
  it('метка закрытого блока — плашка с нужным допуском', () => {
    const w = render('redacted', { level: 6 })
  ...
  it('неизвестный тип блока не ломает страницу; «constructor» и прочие имена из прототипа — тоже', () => {
    for (const type of ['zzz', 'constructor', '__proto__', 'toString', 'hasOwnProperty']) {
      expect(render(type).text()).toContain('не может быть показан')
    }
  })
})
