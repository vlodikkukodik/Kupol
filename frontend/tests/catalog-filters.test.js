...
import CatalogFilters from '../src/components/CatalogFilters.vue'

const summary = { types: [{ type: 'memo', name: 'Меморандум', count: 3 }], departments: [], classes: [] }
...
describe('CatalogFilters: годы', () => {
  ...
  it('когда адрес изменился (назад, сброс), поля показывают значения из адреса', async () => {
    ...
    expect(w.find('#f-from').element.value).toBe('1970')
    expect(w.find('#f-to').element.value).toBe('1990')
    await w.setProps({ query: {} })
    expect(w.find('#f-from').element.value).toBe('')
    expect(w.find('#f-to').element.value).toBe('')
    await w.setProps({ query: { from: '1980' } })
    expect(w.find('#f-from').element.value).toBe('1980')
  })
  ...
  it('незавершённый ввод в одном поле не мешает другому', async () => {
    ...
    expect(w.find('#f-from').element.value).toBe('1975')
    expect(w.find('#f-to').element.value).toBe('19')
  })
})
