...
import { validateLogin, validatePassword } from '../src/lib/rules.js'
...
const valueOf = (v) => (v.input !== undefined ? v.input : String.fromCodePoint(...v.codepoints))
...
describe('validateLogin — общие векторы с сервером', () => {
  ...
  it.each(vectors.map((v) => [JSON.stringify(valueOf(v)), v]))('%s', (_name, v) => {
...
describe('validatePassword', () => {
  ...
  it('отвергает управляющие символы', () => {
    for (const bad of ['passwo\u0000rd', 'pass\nword1', 'pass\tword1']) {
