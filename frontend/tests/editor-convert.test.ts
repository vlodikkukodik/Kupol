...
const block = (type: string, data: unknown, extra: Partial<InputBlock> = {}): InputBlock => ({ id: `${type}1`, type, data, ...extra })

/** Каноническая форма — та, что хранит сервер: её преобразование туда и обратно не меняет ни знака. */
const canonical: InputBlock[] = [
  ...
  block('clipping', { kind: 'newspaper', title: 'Огни', source: 'Вечерний', date: '02.03.1979', paragraphs: [[{ text: 'Свечение.' }], [{ text: 'Без комментариев.' }]] }),
  block('clipping', {
    kind: 'transcript',
    title: 'Запись № 3',
