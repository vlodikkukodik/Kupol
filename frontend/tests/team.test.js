...
let server
let session = { user: null } // что сервер отвечает сейчас
let mode = 'ok' // ok | down
let sessionHits = 0

const ALL = ['team_panel', 'write_drafts', 'review', 'publish', 'edit_published', 'manage_glossary', 'manage_templates', 'manage_team']
const user = (capabilities, extra = {}) => ({
  login: 'tester',
  level: 1,
...
beforeEach(async () => {
  ...
  sessionHits = 0
  session = { user: null }
  const { createAppRouter } = await import('../src/router/index.js')
