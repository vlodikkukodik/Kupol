...
const isLatin = (ch) => (ch >= 'A' && ch <= 'Z') || (ch >= 'a' && ch <= 'z')
const isCyrillic = (ch) => (ch >= 'А' && ch <= 'я') || ch === 'Ё' || ch === 'ё'
...
export function validateLogin(input) {
  const login = input.normalize('NFC')
  const chars = Array.from(login)

  if (chars.length < LOGIN_MIN || chars.length > LOGIN_MAX) {
    return 'Логин: от 3 до 24 символов'
  }

  let seen = null // 'latin' | 'cyrillic'
  for (let i = 0; i < chars.length; i++) {
    ...
    if (script) {
      if (seen && seen !== script) {
        return 'Логин: буквы только латиницей или только кириллицей, не вперемешку'
      }
      seen = script
    }
  }
...
const fold = (s) => s.normalize('NFC').toLowerCase().replaceAll('ё', 'е')
